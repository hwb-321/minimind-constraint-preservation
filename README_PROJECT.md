# MiniMind Calc Prefix-Noise Experiments

本仓库是在 MiniMind 上扩展出的一个实验分支，主要研究小语言模型在 `<calc>` 加法任务中遇到长前缀噪声时的鲁棒性。

上游 MiniMind 的通用说明仍保留在：

- [README.md](/data/chenjingdong/ala/minimind/README.md)
- [README_en.md](/data/chenjingdong/ala/minimind/README_en.md)

本文档只说明当前工作目录里和 `<calc>` 实验直接相关的数据、训练、评测和 test-time attention 干预。所有命令默认从项目根目录 `/data/chenjingdong/ala/minimind` 执行，除非特别说明。
...
- [model/model_minimind.py](/data/chenjingdong/ala/minimind/model/model_minimind.py)：MiniMind 模型实现，包含 RoPE/YaRN 和自定义 attention mask 接口。
- [methods/targeted_interventions.py](/data/chenjingdong/ala/minimind/methods/targeted_interventions.py)：定位 `<calc>` span，并构造 attention-block mask。
- [scripts/eval_calc_dataset.py](/data/chenjingdong/ala/minimind/scripts/eval_calc_dataset.py)：单个数据集评测脚本，支持 baseline、YaRN 和 attention block。
- [scripts/run_attention_block_keep4_9evals.sh](/data/chenjingdong/ala/minimind/scripts/run_attention_block_keep4_9evals.sh)：并行跑 9 组 attention-block 评测。
...
默认权重目录是 [out/](/data/chenjingdong/ala/minimind/out)。评测脚本按下面的规则拼接权重路径：
...
脚本：[scripts/generate_calc_sft_dataset.py](/data/chenjingdong/ala/minimind/scripts/generate_calc_sft_dataset.py)
...
脚本：[scripts/generate_calc_prefix_noise_dataset.py](/data/chenjingdong/ala/minimind/scripts/generate_calc_prefix_noise_dataset.py)
...
脚本：[scripts/generate_calc_prefix_noise_sweep.py](/data/chenjingdong/ala/minimind/scripts/generate_calc_prefix_noise_sweep.py)
...
训练脚本位于 [trainer/](/data/chenjingdong/ala/minimind/trainer)。脚本默认保存 `.pth` 到 `../out`，保存训练断点到 `../checkpoints`。

### 7.1 预训练

脚本：[trainer/train_pretrain.py](/data/chenjingdong/ala/minimind/trainer/train_pretrain.py)
...
脚本：[trainer/train_full_sft.py](/data/chenjingdong/ala/minimind/trainer/train_full_sft.py)
...
核心脚本：[scripts/eval_calc_dataset.py](/data/chenjingdong/ala/minimind/scripts/eval_calc_dataset.py)
...
脚本：[scripts/eval_calc_prefix_noise_sweep.py](/data/chenjingdong/ala/minimind/scripts/eval_calc_prefix_noise_sweep.py)
...
脚本：[scripts/run_attention_block_keep4_9evals.sh](/data/chenjingdong/ala/minimind/scripts/run_attention_block_keep4_9evals.sh)
...
脚本：[scripts/read_jsonl_acc_dir.py](/data/chenjingdong/ala/minimind/scripts/read_jsonl_acc_dir.py)
...
脚本：[scripts/read_result_all_acc.py](/data/chenjingdong/ala/minimind/scripts/read_result_all_acc.py)
...
[model/model_minimind.py](/data/chenjingdong/ala/minimind/model/model_minimind.py) 中模型结构默认会构造更长的 RoPE 表：
